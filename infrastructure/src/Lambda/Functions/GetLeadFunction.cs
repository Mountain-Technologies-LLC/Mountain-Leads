...
public class GetLeadFunction
{
    private readonly IDynamoDbHelper _dynamoDbHelper;

    public GetLeadFunction()
    {
    ...
    public GetLeadFunction(IDynamoDbHelper dynamoDbHelper)
    {
    ...
    public async Task<APIGatewayProxyResponse> FunctionHandler(APIGatewayProxyRequest request, ILambdaContext context)
    {
        try
        {
            context.Logger.LogInformation($"GetLeadFunction invoked");
            ...
            var lead = await _dynamoDbHelper.GetLeadAsync(userId, leadId);
            ...
            {
                context.Logger.LogWarning($"Authorization failed: User {userId} attempted to access lead {leadId} owned by {lead.UserId}");
                return CreateErrorResponse(
            ...
            context.Logger.LogInformation($"Lead retrieved successfully: {leadId}");
